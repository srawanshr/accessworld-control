    ...
    <td class="text-right">
        @if(auth()->user()->canOne(['save.content', 'delete.content']))
            @if(auth()->user()->can('save.content'))
                <a href="{{route('operatingSystem.edit', $operatingSystem->slug)}}" class="btn btn-flat btn-primary">
                    Edit
                </a>
            @endif
            @if(auth()->user()->can('delete.content'))
                <a role="button" href="javascript:void(0);" class="btn btn-flat btn-primary item-delete" data-url="{{ route('operatingSystem.destroy', $operatingSystem->id) }}">
                    Delete
                </a>
            @endif
        @else
