            ...
            <div class="tab-content clearfix">
                <div class="tab-pane" :class="{ 'active' : step == 1 }" id="tab1">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-12">
                            <div class="form-group">
                                <input type="text" name="ftp_password" class="form-control" v-model="provision.ftp_password" required>
                                <label>FTP Password</label>
                                <p class="help-block">Password for the FTP account</p>
                            </div>
                        </div>
                    </div>
                </div><!--end #tab1 -->
    ...
    var vm = new Vue({
        ...
        data: {
            ...
            provision: {
                username: '',
                name: '{{ $webOrder->name }}',
                domain: '{{ $webOrder->domain }}',
                disk: '{{ $webOrder->disk }}',
                traffic: '{{ $webOrder->traffic }}',
                password: '{{ old('password') }}',
                ftp_password: '{{ old('ftp_password') }}',
        ...
        ready: function() {
            this.provision.username = '{{ $webOrder->customer->username }}';
        },
            ...
            submitProvision: function () {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        {
                            bootbox.alert('Web Provision Successful.', function () {
                                window.location.href = '{{ route('order.web.index') }}';
                            });
                        } else {
                            bootbox.alert('Web Provision Not Successful. '+ response.status);
                        }
                    },
                    error: function (e) {
                        $btn.button('reset');
